...
public class AIPath : MonoBehaviour
{
    ...
    /** Holds the transform of the cursor location. Will be used as the target if the user is not
     * navigating to a tag.
     */
	...
	public AudioClip pathFinished;

    public AudioClip waypoint;

	public AudioClip pathAbandoned;
    ...
    public UnityEvent StartCube;

    public UnityEvent EndCube;
	...
	public UnityEvent CollisoinDetection;

	public UnityEvent RemoveTags;
 ...
 */
    ...
    {
        audioSourceContainer = new GameObject("AudioSourceContainer - AIPath", new Type[] { typeof(AudioSource) });
    ...
    public void OnGesture()
    {
        ...
        OnDisable();
        target = cursorTransform;
        OnEnable();
    }

	/// <summary>
	/// Navigate to a selected tag
	/// </summary>
	/// <param name="tag">Tag.</param>
    public void OnTag(GameObject tag)
    {
        OnTagTransform(tag.transform);
    }

	public void OnTagTransform(Transform _target)
	{
		if (AstarPath.active.graphs == null || AstarPath.active.graphs.Length == 0)
		{return;}
    ...
    {
        // pathComlete is set true in OnPathComplete, when Seeker returns a path
        if (pathComplete)
        {
            audioPos = Vector3.Lerp(audioPos, seeker.MoveAudio(), Time.deltaTime);
        ...
        //if (!pathComplete && audioSource.isPlaying)
        //{
        //    audioSource.Stop();
        //}

        // Skip the rest of the code since it is used to move game objects
        if (!canMove) { return; }
	...
	/// <summary>
	/// Stop navigation from voice command
	/// </summary>
