...
namespace HoloToolkit.Unity.SpatialMapping
{
    ...
    public partial class SpatialMappingManager : Singleton<SpatialMappingManager>
    {
		...
		{
//			surfaceMaterial.color = new Color32(255,255,255);
		}
        ...
        /// <summary>
        /// Turn mesh off if it is more than 5 feet away to increase framerate
        /// </summary>
        public void UpdateRendering()
        {
            if (Source != null)
            {
                ...
                for (int index = 0; index < renderers.Count; index++)
                {
                    if (renderers[index] != null)
                    {
                        if (Vector3.Distance(renderers[index].gameObject.transform.position, Camera.main.transform.position) > 10)
