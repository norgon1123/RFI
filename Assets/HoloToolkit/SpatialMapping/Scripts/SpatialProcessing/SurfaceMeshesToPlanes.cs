...
namespace HoloToolkit.Unity.SpatialMapping
{
    ...
    public class SurfaceMeshesToPlanes : Singleton<SurfaceMeshesToPlanes>
    {
        ...
        /// <summary>
        /// Holds reference to astar data
        /// </summary>
        private AstarData data;

        /// <summary>
        /// Active grid graph in scene
        /// </summary>
        ...
        /// <summary>
        /// Creates GridGraph for A* pathfinding based
        /// on floor dimensions
        /// </summary>
        private void CreateGraph(SurfacePlane plane)
        {
            ...
            // Setup a grid graph with the dimensions of the floor plane
            gg.nodeSize = 0.2f;
            gg.width = Mathf.RoundToInt(plane.transform.localScale.x / gg.nodeSize + 4);
            gg.depth = Mathf.RoundToInt(plane.transform.localScale.y / gg.nodeSize + 4);
            gg.center = plane.transform.position;
            gg.center.y -= 0.1f; // Avoid floating point errors by creating a small buffer
            gg.rotation = plane.transform.rotation.eulerAngles;
            gg.rotation = new Vector3(0, gg.rotation.y, 0); // Fix the rotation
            gg.maxSlope = 45f;

            // Set the collision settings
            gg.collision.mask += LayerMask.GetMask("SpatialMapping");
            gg.collision.mask += LayerMask.GetMask("Unwalkable");
            gg.collision.heightMask = LayerMask.GetMask("Ground");
            gg.collision.type = ColliderType.Capsule;
            gg.collision.height = 1.25f;
            gg.collision.diameter = 200;

            // Updates internal size from the above values, but wait to scan it in SurfaceMeshesToPlanes.cs
        ...
        {
			// Delete Tables in TableCollision to avoid null references
			//GetComponent<TableCollision>().ClearTables();
            ...
            if (gg != null)
            {
                AstarData.active.astarData.RemoveGraph(gg);
            }
            ...
            for (int index = 0; index < planes.Length; index++)
            {
                ...
                else
                {
                    ...
                    if (destPlane.layer == LayerMask.NameToLayer("Default"))
                    {
                ...
                if (planes[index].Bounds.Center.y == FloorYPosition)
                {
                    CreateGraph(surfacePlane);
                }
            }
            ...
            // Start the observer again and start refreshing the A* grid if not already
            RefreshScan.Instance.StartScan();
        }
